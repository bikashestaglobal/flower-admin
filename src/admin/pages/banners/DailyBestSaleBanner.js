...
function DailyBestSaleBanner() {
  ...
  const imageChangeHandler = (event) => {
    ...
    if (file && file[0]) {
      ...
      image.onload = async function () {
        ...
        if (height == 769 && width == 540) {
          handleUpload(file[0]);
        } else {
          M.toast({
            html: "Please Upload Image Size 540 X 769",
            classes: "bg-danger",
          });
          return;
        }
    ...
    fileRef
      ...
      .then(() => {
        ...
        fetch(`${Config.SERVER_URL}/setting`, {
          method: "PUT",
          body: JSON.stringify({ bestSaleBanner: { ...formData, image: "" } }),
          headers: {
  ...
  const submitHandler = (evt) => {
    ...
    fetch(Config.SERVER_URL + "/setting", {
      method: "PUT",
      body: JSON.stringify({ bestSaleBanner: { ...formData } }),
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${localStorage.getItem("jwt_branch_token")}`,
      },
  ...
  useEffect(() => {
    fetch(`${Config.SERVER_URL}/setting`, {
      ...
      .then(
        (result) => {
          if (result.status == 200) {
            if (result.body.bestSaleBanner) {
              const { title, image, webpageUrl } = result.body.bestSaleBanner;
              setFormData({
                ...formData,
                title: title,
                image: image,
  ...
  return (
    <div className="page-wrapper px-0 pt-0">
      <div className={"container-fluid"}>
        ...
        <Breadcrumb title={"DAILY BEST SALE BANNER"} pageTitle={"Add Banner"} />
...
export default DailyBestSaleBanner;
