...
const EditPincodeFromCSV = () => {
  ...
  const submitHandler = ({ id, pincode, state, city, status }) => {
    const updateData = {
  ...
  const downloadCSVHandler = () => {
    ...
    makeElement("th", "pincode", row);
    makeElement("th", "state", row);
    makeElement("th", "city", row);
        ...
        (result) => {
          ...
          if (result.status === 200) {
            result.body.map((item, index) => {
              ...
              makeElement("td", item.pincode, dataRow);
              makeElement("td", item.state, dataRow);
              makeElement("td", item.city, dataRow);

              makeElement("td", item.status, dataRow);

              thead.appendChild(dataRow);
            });
            tableToCSV("pincodes.csv", table);
...
export default EditPincodeFromCSV;
