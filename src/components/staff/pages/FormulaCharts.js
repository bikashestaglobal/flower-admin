...
//  Component Function
const FormulaCharts = React.memo((props) => {
  ...
  // const [isallFormulaChartsLoaded, setIsallFormulaChartsLoaded] = useState(true);
  ...
  const deleteSubmitHandler = async () => {
    ...
    fetch(Config.SERVER_URL + "/staff/deleteFormulaChart", {
      method: "DELETE",
      body: JSON.stringify(FormulaChartData),
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${localStorage.getItem("jwt_staff_token")}`,
      },
  ...
  const submitHandler = (evt) => {
    ...
    fetch(Config.SERVER_URL + "/staff/addFormulaChart", {
      method: "POST",
  ...
  useEffect(() => {
    // setIsallFormulaChartsLoaded(true);
    fetch(Config.SERVER_URL + "/staff/searchFormulaChart?session=" + selectedSession, {
      method: "GET",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${localStorage.getItem("jwt_staff_token")}`,
      ...
      .then(
        (result) => {
          // setIsallFormulaChartsLoaded(false);
          if (result.success) {
      ...
      Cell: ({ row }) => {
        ...
        return (
          <div>
            ...
            {/* update button */}
            {state && state.classRoomAsset.updateFormulaChart ?
              <button
                type="button"
            ...
            {/* Show Button */}
            {state && state.classRoomAsset.showSingleFormulaChart ?
              <button
                type="button"
            ...
            {/* Delete Buttons */}
            {state && state.classRoomAsset.deleteFormulaChart ? <button
              type="button"
            ...
            <div className={"card mb-0 mt-2 border-0 rounded-0"}>
              <div className={"card-body pb-0 pt-2"}>
                <div>
                  ...
                  {state && state.classRoomAsset.addFormulaChart ? <button
                    type="button"
        ...
        <div>
          ...
          <div className="modal fade" id="viewModal">
            <div className="modal-dialog modal-lg" role="document">
              <div className="modal-content rounded-0">
                <div className={"modal-header"}>
                  <h4 className={"float-left"}>View Formula Charts</h4>
                </div>
                <div className="modal-body">
                  {/* Information */}
                  <div className={"row"}>
