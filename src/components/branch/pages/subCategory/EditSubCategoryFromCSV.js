...
const EditSubCategoryFromCSV = () => {
  ...
  const submitHandler = ({
    ...
    const updateData = {
      ...
      parentCategories: parentCategories || undefined,
  ...
  const downloadCSVHandler = () => {
    ...
    fetch(`${Config.SERVER_URL}/category?skip=0&limit=100000`, {
      method: "GET",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${localStorage.getItem("jwt_branch_token")}`,
        ...
        (result) => {
          ...
          if (result.status === 200) {
            result.body.map((item, index) => {
              const pCat = item.parentCategories.map((pItem) => {
                return `${pItem.id}`;
              });
              ...
              makeElement("td", pCat.join("_"), dataRow);
...
export default EditSubCategoryFromCSV;
