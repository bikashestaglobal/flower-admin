...
const EditCouponFromCSV = () => {
  ...
  const downloadCSVHandler = () => {
    ...
    fetch(`${Config.SERVER_URL}/coupon?skip=0&limit=0`, {
      method: "GET",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${localStorage.getItem("jwt_branch_token")}`,
        ...
        (result) => {
          ...
          if (result.status === 200) {
            result.body.map((item, index) => {
              ...
              makeElement("td", item.applyFor, dataRow);
              makeElement("td", item.discountType, dataRow);
              makeElement("td", item.discount, dataRow);
              makeElement("td", item.description, dataRow);
              makeElement("td", item.minimumAmount, dataRow);
...
export default EditCouponFromCSV;
