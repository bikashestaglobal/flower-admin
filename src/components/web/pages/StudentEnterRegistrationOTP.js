...
function StudentEnterRegistrationOTP() {
  ...
  const [studentForRegistration, setStudentForRegistration] = useState(JSON.parse(localStorage.getItem("studentForRegistration")) || {student_mobile:"", otp:""});

  
  const verifySubmitHandler = (evt)=>{
    evt.preventDefault();
    setIsVerifyLoaded(false)
    const sendOTP = Number(studentForRegistration.otp)-2021;
    const enteredOTP = otp;
    // If lOcalstorage is Empty
    if(studentForRegistration.otp == ""){
        M.toast({ html: "First Fill Registration Form!", classes: "bg-danger" });
    ...
    if(sendOTP == enteredOTP){
        ...
        setIsVerifyLoaded(false);
        fetch(Config.SERVER_URL+"/student/registration", {
          method: "POST",
          body: JSON.stringify(studentForRegistration),
          headers: {
            ...
            (result) => {
              ...
              if (result.success) {
                if(result.msg == "send"){
                  ...
                  localStorage.removeItem("studentForRegistration");
  ...
  const resendSubmitHandler = (evt) => {
    evt.preventDefault();
    // If lOcalstorage is Empty
    if(studentForRegistration.student_mobile == ""){
    ...
    const studentData = {
      student_mobile: studentForRegistration.student_mobile
    };
      ...
      .then(
        (result) => {
          ...
          if (result.success) {
            if(result.msg == "send"){
              M.toast({ html: result.message, classes: "bg-success" });
              localStorage.setItem("studentForRegistration", JSON.stringify({...result.data, verify: false}));
            }else{
              M.toast({ html: result.message, classes: "bg-danger" });
            }
          } else {
...
export default StudentEnterRegistrationOTP;
